...
{
    $number = trim($number);
    return preg_match("/^(\+977)?(98|97)[0-9]{8}$/", $number);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (validateName($name) !== true) {
        ...
        $error = "Invalid Nepal mobile number.";

    } elseif (empty($email) || empty($role)) {
        $error = "Name, Email and Role are required.";

    } elseif (!preg_match("/^[a-zA-Z0-9._%+-]+@[a-zA-Z][a-zA-Z0-9.-]*\.[a-zA-Z]{2,}$/", $email)) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            if ($id) {
                ...
                if (!empty($password)) {
                    ...
                    $stmt->bind_param("sssssi", $name, $email, $phone, $hash, $role, $id);
                } else {
                    $stmt = $conn->prepare(
            ...
            /* -------- INSERT -------- */ else {
                ...
                if ($stmt->execute()) {
                    ...
                    if ($role === 'doctor') {
                        $stmt2 = $conn->prepare(
                            "INSERT INTO doctors (user_id, specialization) VALUES (?, 'General')"
                        );
                    ...
                    if ($role === 'patient') {
                        ...
                        $stmt2->bind_param("is", $new_id, $name);
                        $stmt2->execute();
                    }
                } else {
                        ...
                        <div class="card-body">
                            <form method="POST">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="tel" name="phone" class="form-control" pattern="\(98|97)[0-9]{8}"
                                        maxlength="10" required placeholder="98XXXXXXXX">
                                </div>
                                <div class="form-group">
                        ...
                        <div class="card-header">
                            ...
                            <form method="GET" class="filter-bar">
                                ...
                                <select name="role_filter" class="filter-select">
                                    ...
                                    <option value=a"doctor" <?= $role_filter === 'doctor' ? 'selected' : '' ?>>Doctor
                                    </option>
                                    <option value="patient" <?= $role_filter === 'patient' ? 'selected' : '' ?>>Patient
                                    </option>
                                </select>
                                <button type="submit" class="btn-filter"><i class="fas fa-filter"></i></button>
                                <?php if (!empty($search) || !empty($role_filter)): ?>
                                    <a href="manage-users.php" class="btn-filter"
                                        style="background:#64748b; text-decoration:none;"><i class="fas fa-times"></i></a>
                                <?php endif; ?>
                            </form>
                        </div>
                            ...
                            <table class="users-table">
                                ...
                                <tbody>
                                    ...
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="4"
                                                style="text-align:center; padding: 2rem; color:var(--text-muted);">No users
                        ...
                        <?php if ($total_pages > 1): ?>
                            <div class="pagination">
                                ...
                                <div class="page-links">
                                    <a href="?page=<?= max(1, $page - 1) ?>&search=<?= urlencode($search) ?>&role_filter=<?= urlencode($role_filter) ?>"
                                        class="page-link <?= $page <= 1 ? 'disabled' : '' ?>">Prev</a>

                                    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                                        <a href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&role_filter=<?= urlencode($role_filter) ?>"
                                            class="page-link <?= $i == $page ? 'active' : '' ?>"><?= $i ?></a>
                                    <?php endfor; ?>

                                    <a href="?page=<?= min($total_pages, $page + 1) ?>&search=<?= urlencode($search) ?>&role_filter=<?= urlencode($role_filter) ?>"
        ...
        <div class="modal-card">
            ...
            <div class="card-body">
                <form method="POST" id="editForm">
                    ...
                    <div class="form-group">
                        <label>New Password <span style="font-weight:400; color:var(--text-muted);">(Leave blank to keep
                                current)</span></label>
